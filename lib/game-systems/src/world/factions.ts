 ...
 *
 * Shared SSOT for api-server, website, and arpg-game. Client re-exports
 * from `@workspace/game-systems/world/factions` so reputation, sectors, and
 * grudge membership never drift.
 */
...
export interface FactionDef {
  ...
  name: string;
  shortName: string;
  /** Territory id from sectors catalog. One faction owns one named territory. */
  territoryId: string;
  ...
  creed: string;
  /** Playstyle signature (info.html distilled). */
  signature: string;
}
...
export const REP_TIERS: RepTierDef[] = [
  {
    ...
    min: -100,
    max: -76,
    description: 'Attack on sight anywhere. Bounty posted. Named hunter dispatched.',
  },
  {
    id: 'hostile',
    label: 'Hostile',
    min: -75,
    max: -26,
    description: 'Attack on sight in territory. No dialog/trade/quests.',
  },
  {
    id: 'unfriendly',
    label: 'Unfriendly',
    min: -25,
    max: -1,
    description: 'Will not attack first but refuses trade.',
  },
  {
    id: 'neutral',
    label: 'Neutral',
    min: 0,
    max: 24,
    description: 'Coexistence. Base prices. Default starting state.',
  },
  {
    id: 'friendly',
    label: 'Friendly',
    min: 25,
    max: 49,
    description: '-10% trade. Standard contracts. Guards warn off hostiles nearby.',
  },
  {
    id: 'honored',
    label: 'Honored',
    min: 50,
    max: 74,
    description: '-20% trade. Guards intervene. Signature recipes unlock.',
  },
  {
    id: 'allied',
    label: 'Allied',
    min: 75,
    max: 99,
    description: 'Patrols defend your camps. -30% trade. Caravans every 3-5 days.',
  },
  {
    id: 'hero',
    label: 'Hero',
    ...
    max: 100,
    description:
      'Faction reinforces you in raids. Champion NPC joins. Max one Hero at a time.',
  },
];
...
export function newReputationVector(initial = 0): ReputationVector {
  return {
    keepers: initial,
    tech_scavengers: initial,
    hollow_lords: initial,
    network: initial,
    forgotten: initial,
  };
}
