...
function TopNav() {
  ...
  {/* Mobile menu button */ }
  <button className="md:hidden text-secondary p-1" onClick = {() => setOpen(!open)
...
{/* Mobile dropdown */ }
{
  open && (
    <div className="absolute top-full left-0 right-0 bg-background/95 backdrop-blur-md border-b border-border/40 py-4 flex flex-col gap-4 px-6 text-sm font-serif tracking-[0.2em] uppercase md:hidden" >
      <a href="#factions" className = "text-muted-foreground hover:text-secondary" onClick = {() => setOpen(false)
}> Factions </a>
...
function BiomesSection() {
  ...
  <span className="text-secondary" > None of them safe.</span>
    ...
    < div className = "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4" >
    {
      ...
      >
      <h3 className="font-serif text-xl mb-2" style = {{ color: b.color }
}> { b.name } </h3>
