...
function placeBuilding(scene: THREE.Scene, bx: number, bz: number) {
  ...
  // Walls
  addMesh(scene, GEO.box(w, bh, d), MAT.stone, bx, h + bh * 0.5, bz, Math.random() * 0.5);
  // Roof
...
function placeCaveEntrance(scene: THREE.Scene, cx: number, cz: number) {
...
function placeOutpost(scene: THREE.Scene, bx: number, bz: number) {
  const h = worldHeight(bx, bz);
  // Watch tower
...
function buildTown(scene: THREE.Scene, def: SettlementDef) {
  const { x, z } = def;
  ...
  for (let i = 0; i < bCount; i++) {
    ...
    placeBuilding(scene, bx, bz);
  }

  // A couple extra tents
  for (let i = 0; i < 3; i++) {
...
export class FeaturePlacer {
  ...
  buildAll() {
    ...
    for (const def of settlements) {
      switch (def.type) {
        case 'town':    buildTown(this.scene, def);          break;
        case 'camp':    buildCamp(this.scene, def);          break;
        case 'cave':    placeCaveEntrance(this.scene, def.x, def.z); break;
        case 'outpost': placeOutpost(this.scene, def.x, def.z);      break;
      }
    }
  }
