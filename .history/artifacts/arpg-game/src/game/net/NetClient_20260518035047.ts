...
export class NetClient {
   ...
   * api-server).  In local dev the var is unset and we fall back to the
   * same origin (works because `dev:api` runs alongside `dev:game`).
   *
   * Set VITE_WS_URL=wss://grudge-nexus-api-production.up.railway.app
   * (or whatever the Railway public domain is) in your Vercel project env.
   */
  private resolveUrl(): string {
    const explicit = import.meta.env.VITE_WS_URL as string | undefined;
    if (explicit) {
      // Strip trailing slash and append path.
      return explicit.replace(/\/$/, '') + '/api/realtime';
    }
