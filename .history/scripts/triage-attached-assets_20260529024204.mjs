...
const SKIP_PATTERNS = [
  ...
  /^smarts-achievements/i,   // same family as above
  /^VFXFootageExplosion/i,   // 193 MB of video footage, not three.js-loadable
];
...
for (const k of keepers) {
  ...
  try {
    // tar.exe (ships with Win10+) handles brackets, apostrophes, and spaces
    // in paths far better than PowerShell's Expand-Archive.
    execFileSync('tar.exe', ['-xf', k.path, '-C', destDir], { stdio: 'inherit' });
