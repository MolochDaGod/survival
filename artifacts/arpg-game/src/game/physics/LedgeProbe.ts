...
import { GROUPS_PROBE_WORLD } from './PhysicsGroups';
...
/** Minimum rise we treat as a "ledge". Matches the KinematicCharacterController's
 *  `enableAutostep(0.45, …)` ceiling exactly so there is no dead zone between
 *  "auto-stepped invisibly" and "scripted mantle" — anything taller than the
 *  controller can handle on its own immediately triggers a climb instead of
 *  wedging the capsule against the rise. */
const MIN_RISE = 0.45;
