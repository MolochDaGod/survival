...
}> = ({ gender, nameInput, onGenderChange, onNameChange }) => (
  <div>
    ...
    <div style={{
      ...
      fontSize: 11, color: '#8899aa', lineHeight: 1.5,
    }}>
      Body shape (height, build) is in the <span style={{ color: '#e8a030' }}>Looks</span> tab.
      Clothing and armor come from gear you find and equip in the world.
    </div>

    <SectionLabel>Starting Gear</SectionLabel>
    <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 7, padding: 10, marginBottom: 4 }}>
      {['Basic Shirt', 'Basic Pants', 'Basic Shoes'].map(item => (
        <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#8899aa', flexShrink: 0 }} />
          <span style={{ fontSize: 12, color: '#8899aa' }}>{item}</span>
          <span style={{ marginLeft: 'auto', fontSize: 10, color: '#556677', fontStyle: 'italic' }}>equipped</span>
        </div>
      ))}
      <div style={{ fontSize: 10, color: '#445566', marginTop: 6, lineHeight: 1.5 }}>
        Additional gear is found in-game.
      </div>
    </div>
  </div>
);
